...
class fzm(Command):
    ...
    """
    This for in progress stuff:
    https://github.com/ranger/ranger/blob/master/ranger/config/commands.py
    """
    def tab(self, whatever):
        home = os.environ.get("HOME", os.path.expanduser("~"))
        logfile = os.path.join(home, "poop")

        if not os.path.exists(logfile):
            os.mknod(logfile)

        mark_file = os.path.join(home, ".fzf-marks")
        mark_file = os.environ.get("FZF_MARKS_FILE", mark_file)

        with open(mark_file) as f:
            content = f.readlines()
            arguments = self.rest(1)

            matches = filter(curry_find(arguments), content)

        # with open(logfile, "a") as myfile:
        #     newline = ""
        #     myfile.write(newline.join(matches))

        return matches
        # return ("lalalala" for y in matches)
        # for y in matches:
        #     yield y
